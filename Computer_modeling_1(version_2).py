...
def genetic_algorithm(population):
    while True:
        ...
        rand1 = rand2 = 0
        while rand1 == rand2:
            rand1 = population[lengths.index(*random.choices(lengths, weights))]
            rand2 = population[lengths.index(*random.choices(lengths, weights))]



        # Скрещивание
        l = random.randint(0, len(rand1) - 1)

        new_1 = rand1[:l]
        new_2 = rand2[:l]
        for i in range(len(rand1)):
            if rand2[i] not in new_1:
                new_1.append(rand2[i])

            if rand1[i] not in new_2:
                new_2.append(rand1[i])
        ...
        i1 = i2 = 0
        while i1 == i2:
            i1 = random.randint(0, len(rand1) - 1)
            i2 = random.randint(0, len(rand1) - 1)
...
for test in ('test_1(29).tsp', 'test_2(58).tsp', 'test_3(561).tsp'):
    ...
    initial_solution = [i for i in range(len(cities) - 1, -1, -1)]
